...
<summary>復習 & 整理したいことなど</summary>
  ...
  1.  `self`がなくてもインスタンスメソッドからインスタンス変数参照できそう
       <details>
         ...
         - [cf.](https://qiita.com/fumi1011/items/484a4366ca424e06345e)
           ...
           - インスタンス変数はコレ`@hoge`
           - self.hogeはattr_accessorを経由して間接的に`@hoge`にアクセスしてる
           - 推測だが、ローカル変数のスコープ > インスタンス変数のスコープとなっていて、  
             明示的に`self.hoge`としない場合(単に`hoge`とする場合)、  
             ローカル変数の`hoge`がないなら、インスタンス変数の`hoge`として解釈してそう
       </details>
  ...
  1.  定義しなくても使えるメソッド達はなに。rubyのdocとかにメソッド仕様ある？
       <details>
